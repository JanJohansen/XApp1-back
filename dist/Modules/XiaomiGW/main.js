...
let log = (x) => db.upsert({ _id: "XiomiGW_1", outs: { _log: x } });
let I = {};
class XiaomiGW {
    constructor() {
        this.setup();
                ...
                switch (device.getType()) {
                    ...
                    case "sensor":
                        ...
                        device.on("update", () => {
                            ...
                            if (press)
                                db.upsert({ _id: "XiaomiGW:" + device.getSid(), outs: { temperature: { v: temp } }, humidity: { v: hum }, pressure: { v: press } });
                            else
                                db.upsert({ _id: "XiaomiGW:" + device.getSid(), outs: { temperature: { v: temp } }, humidity: { v: hum } });
                        });
                        break;
                    case "leak":
