...
export class NodeImplementation extends ModelInstance {
	...
	scriptSetup: (() => Promise<void>) | null = null
	scriptCleanup: (() => Promise<void>) | null = null
	registerSetupFunction = (scriptSetup: () => Promise<void>) => {
		this.scriptSetup = scriptSetup
	}
	registerCleanupFunction = (scriptCleanup: () => Promise<void>) => {
	...
	async setup() {
		...
		this.on("ins.code", async (code) => {
			...
			if (this.script) {
				try {
					this.script(
						self.setType.bind(self),
						this.registerSetupFunction.bind(self),
						self.on.bind(self),
						self.set.bind(self),
						console.log,
						this.registerCleanupFunction.bind(self)
					)
				} catch (e) {
			...
			// Run setup function definition from script!
			if(this.scriptSetup) await this.scriptSetup()
		}
	}
	async cleanup(): Promise<void> {
		super.cleanup()
		if (this.scriptCleanup) await this.scriptCleanup()
	}
}
